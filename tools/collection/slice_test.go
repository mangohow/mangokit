...
func TestStream(t *testing.T) {
	...
	t.Run("StreamOf", func(t *testing.T) {
		data := []int{1, 2, 3}
		s := StreamOf(data)
		if cap(s.ss) != cap(data) {
			t.Error("StreamOf should reuse underlying array")
		}
	})

	// 完整流式操作测试
	t.Run("FullPipeline", func(t *testing.T) {
		result := StreamOf([]int{1, 2, 3, 4, 5}).
			Filter(func(n int) bool { return n%2 == 0 }).
			Find(func(n int) bool { return n > 2 }).
			Collect()

		if !reflect.DeepEqual(result, []int{4}) {
			t.Errorf("Pipeline result = %v, want [4]", result)
		}
	})
}
...
func TestStream_Filter(t *testing.T) {
	t.Run("basic filtering", func(t *testing.T) {
		s := StreamOf([]int{1, 2, 3, 4}).Filter(func(n int) bool { return n > 2 })
		if got := s.Collect(); !reflect.DeepEqual(got, []int{3, 4}) {
			t.Errorf("Filter() = %v, want [3,4]", got)
		}
	})

	t.Run("empty after filter", func(t *testing.T) {
		s := StreamOf([]string{"a", "b"}).Filter(func(s string) bool { return false })
		if len(s.Collect()) != 0 {
			t.Error("Filter() should produce empty slice")
		}
	})
	...
	t.Run("filter objects", func(t *testing.T) {
		data := []Person{
			{age: 18},
			{age: 19},
			{age: 20},
		...
		res := StreamOf(data).Filter(func(p Person) bool { return p.age < 20 }).Collect()
		if !reflect.DeepEqual(res, want) {
			t.Errorf("Filter() want: %v, got: %v", want, res)
		}
	})

	t.Run("filter objects ptr", func(t *testing.T) {
		data := []*Person{
			{age: 18},
			{age: 19},
			{age: 20},
...
func TestStream_ForEach(t *testing.T) {
	t.Run("processing order", func(t *testing.T) {
		var log []int
		StreamOf([]int{3, 1, 4}).ForEach(func(n int) bool {
			log = append(log, n)
			return true
		})
		if !reflect.DeepEqual(log, []int{3, 1, 4}) {
			t.Errorf("ForEach order %v, want [3,1,4]", log)
		}
	})

	t.Run("early termination", func(t *testing.T) {
		count := 0
		StreamOf(make([]int, 100)).ForEach(func(int) bool {
			count++
			return count < 5
		})
		if count != 5 {
			t.Errorf("Processed %d elements, want 5", count)
		}
	})
}
...
func TestStream_Find(t *testing.T) {
	t.Run("normal find", func(t *testing.T) {
		...
		s := StreamOf(data).Find(func(n int) bool { return n == 8 })
		if result := s.Collect(); len(result) != 1 || result[0] != 8 {
			t.Errorf("Find() = %v, want [8]", result)
		}
	})

	t.Run("no matches", func(t *testing.T) {
		s := StreamOf([]float64{1.1, 2.2}).Find(func(f float64) bool { return f > 3 })
		if len(s.Collect()) != 0 {
			t.Error("Find() should return empty slice")
		}
	})
}
...
func TestStream_FindFirst(t *testing.T) {
	t.Run("find existing", func(t *testing.T) {
		s := StreamOf([]string{"a", "b", "c"}).FindFirst(func(s string) bool { return s > "a" })
		if result := s.Collect(); !reflect.DeepEqual(result, []string{"b"}) {
			t.Errorf("FindFirst() = %v, want [b]", result)
		}
	})

	t.Run("not found", func(t *testing.T) {
		s := StreamOf([]int{1, 2, 3}).FindFirst(func(n int) bool { return n > 3 })
		if s.Collect() != nil {
			t.Error("FindFirst() should return nil")
		}
	})
